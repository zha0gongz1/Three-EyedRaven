...
var rootCmd = &cobra.Command{
	...
	Long:  "[Notice]:\n[*]Three-EyedRaven is only used as detection and blasting tool for intranet penetration, doesn't contain any exploit code!",
...
var allCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		plugin.AllFunc(&all.Hosts, &all.Ports, &all.NoPing, &all.NoWebDetect, &all.NoBrute, &all.Thread)
...
var detectCmd = &cobra.Command{
	Use:   "detect",
	Short: "[*]Hosts and ports detection module.",
	Run: func(cmd *cobra.Command, args []string) {
		if detect.Hosts == "" {
			_ = cmd.Help()
			return
...
var bruteCmd = &cobra.Command{
	Use:   "brute",
	Short: "[*]Blasting port services model.",
	Run: func(cmd *cobra.Command, args []string) {
		if burp.Hosts == "" {
			_ = cmd.Help()
			return
...
func init() {
	...
	allCmd.Flags().StringVarP(&all.Hosts, "hosts", "H", "", "设置扫描的目标参数(.eg [-H 192.168.233.1] [-H 172.16.1.1/16])")
	allCmd.Flags().IntVarP(&all.Thread, "thread", "t", 400, "设置扫描时的扫描线程 (.eg [-t 500]) 默认400线程")
	...
	detectCmd.Flags().IntVarP(&detect.Thread, "thread", "t", 400, "设置扫描时的扫描线程 (.eg [-t 500]) 默认400线程")
	...
	detectCmd.Flags().StringVarP(&detect.Ports, "ports", "P", "", "设置扫描的端口参数(.eg 80,443-445) 不设置将采用默认top 1000端口字典")

	rootCmd.AddCommand(bruteCmd)
	bruteCmd.Flags().StringVarP(&burp.Hosts, "hosts", "H", "", "设置扫描的目标参数(.eg [-H 192.168.233.1] [-H 172.16.1.1/16])")
	bruteCmd.Flags().StringVarP(&burp.Service, "service", "S", "", "设置爆破的服务名称(.eg -S mysql)\n[mssql,ftp,ssh,mysql,rdp,postgres,redis,winrm,smb,mongodb]")
	bruteCmd.Flags().IntVarP(&burp.Thread, "thread", "t", 200, "设置扫描时的扫描线程 (.eg [-t 500]) 默认200线程")
